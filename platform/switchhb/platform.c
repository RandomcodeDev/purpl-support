...
--*/
{
    ...
    if (!strlen(Buffer))
    {
        UINT32 HorizonOsVersion = hosversionGet();
        snprintf(Buffer, PURPL_ARRAYSIZE(Buffer), "Horizon OS %u.%u.%u%s", HOSVER_MAJOR(HorizonOsVersion),
                 HOSVER_MINOR(HorizonOsVersion), HOSVER_MICRO(HorizonOsVersion),
...
{
    ErrorApplicationConfig ErrorConfig = {0};
    errorApplicationCreate(&ErrorConfig, Message, Message);
